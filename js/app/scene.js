...
class Scene {
  ...
  constructor(scene_config, gl, shader, light_shader) {
    ...
    this.models =
      "models" in scene_config ? this.loadModels(scene_config.models) : {};

    // Then load all the lights and reset the lights in the shader
    this.lights =
      "lights" in scene_config ? this.loadLights(scene_config.lights) : {};
    this.light_counts = {};
    this.resetLights(shader);

    // Load the scenegraph recursively
    this.scenegraph = this.loadScenegraphNode(
      scene_config.scenegraph,
      gl,
      shader,
      light_shader
    );
    ...
    switch (light.type) {
      case "ambient":
        return new AmbientLight(
          this.light_counts[this.lights[name].type],
          color,
          light.intensity,
          shader,
          gl,
  ...
  loadScenegraphNode(node_config, gl, shader, light_shader) {
    ...
    if ("children" in node_config) {
      for (let child of node_config.children) {
        ...
        let child_node = this.loadScenegraphNode(
          child,
          gl,
          shader,
          light_shader
  ...
  getNode(name) {
    ...
    if (node == null) throw `Node "${name}" not found in scenegraph`;
    return node;
  }
...
class SceneNode {
  ...
  getNodes(nodes) {
    nodes.push(this);
    for (let child of this.children) child.getNodes(nodes);
    return nodes;
  }
